...
def SortPages(pagelist):
    changed = True
    while changed:
        changed = False
        for n in range(len(pagelist)-1):
            # compare each pair and swap if required
            a, b = pagelist[n], pagelist[n+1]
            
            # Does a come after b?
            if pagelist[n+1] in followed_by and pagelist[n] in followed_by[pagelist[n+1]]:
                pagelist[n], pagelist[n+1] = b, a
                changed = True

            # Does b come before a?
            elif pagelist[n] in preceeded_by and pagelist[n+1] in preceeded_by[pagelist[n]]:
                pagelist[n], pagelist[n+1] = b, a
                changed = True
    return pagelist
